...
| `manifest_complete`    | **SOFT CONVERSION.** Visitor submitted the cart-drawer "Manifest complete" form (email required). No real checkout exists on Pause and Build — this is the proxy. Mark as a Conversion Event in GA4 admin. Carries the full ecommerce payload (currency, value, items). |
