...
| `manifest_complete`             | **SOFT CONVERSION (drawer).** Visitor submitted the cart-drawer "Manifest complete" form (email required). Carries the full ecommerce payload.                                                                                         |
| `checkout_view`                 | Visitor landed on the faux-checkout page (`/checkout`). Funnel step distinct from `cart_drawer_open`.                                                                                                                                  |
| `checkout_express_click`        | Click on one of the two express CTAs on the checkout. Carries `variant: book_now \| email_direct`.                                                                                                                                     |
| `book_call`                     | **PRIMARY CONVERSION (form-side).** Visitor submitted the faux-checkout form with email + role + Shopify plan + would_have_paid. Carries the full ecommerce payload + qualifier dimensions. Mark as Conversion Event in GA4 admin.     |
| `call_booked_confirmation_view` | **PRIMARY CONVERSION (page-side, recommended).** Page-view of `/checkout/thank-you`. Only fires when the submit actually routed; dedupes naturally on `order_number`. Use this — not `book_call` — as the canonical conversion in GA4. |
